 ...
 */
package com.github.harbby.sylph.colltroller.utils;
            ...
            throws IOException, URISyntaxException
    {
        ...
        if (user != null && !user.isEmpty()) {
            httpRequest.setHeader("Cookie",
                    PROXY_USER_COOKIE_NAME + "=" + URLEncoder.encode(user, StandardCharsets.US_ASCII.name()));
        }
        ...
        try (CloseableHttpClient client = HttpClients.custom().setDefaultRequestConfig(defaultRequestConfig).build()) {
            ...
            if (httpResp.getStatusLine().getStatusCode() == HttpStatus.SC_MOVED_TEMPORARILY) { //302
                BufferedHeader header = (BufferedHeader) httpResp.getFirstHeader("Location");
                proxyLink(req, resp, new URI(header.getValue()), null);
            }
            else if (httpResp.getStatusLine().getStatusCode() == HttpStatus.SC_NOT_MODIFIED) { //304
                resp.setStatus(304);
            }
            else {
