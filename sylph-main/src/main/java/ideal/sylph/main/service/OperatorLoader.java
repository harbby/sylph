...
{
    private static final Logger logger = LoggerFactory.getLogger(OperatorLoader.class);
    ...
    private static <T> Set<Class<? extends T>> loadOperators(Class<T> pluginClass, ClassLoader pluginClassLoader)
            throws IOException, NoSuchMethodException, InvocationTargetException, IllegalAccessException
    {
        ...
        while (configs.hasMoreElements()) {
            ...
            Iterator<String> iterator = (Iterator<String>) method.invoke(serviceLoader, Operator.class, url);
            iterator.forEachRemaining(x -> {
                Class<?> javaClass = null;
                try {
                    javaClass = Class.forName(x, false, pluginClassLoader);  // pluginClassLoader.loadClass(x)
                    if (Operator.class.isAssignableFrom(javaClass)) {
